...
struct op_check
{
    enum { value = priv::supports_ops<A>::value && priv::supports_ops<B>::value && !priv::Is_same<A, priv::JobOp>::value };
};
...
// Also, we use 2 continuations slots everywhere. Why? (A >> (B >> C) >> D) adds 2 continuations to B.
...
private:
    ...
    {
        return tws_newJob(NULL, NULL, 0, 2, tws_TINY, NULL, ev);
    }
    ...
    void addpar(tws_Job *jj)
    {
        ...
        {    // (We can't submit those right away so gotta store them somewhere until it's time to submit them.)
            ...
            size_t idx = 0;
            par[idx++] = _jp ? _jp : _j; // Store the prev. launcher if present, and launch it in parallel with the rest. Alternatively, launch original job.
            par[idx++] = jj;
            _paridx = idx;
            for( ; idx < NumPar; ++idx)
                par[idx] = NULL;
...
public:
    ...
    JobOp(const JobOp& j) : Base(const_cast<JobOp&>(j).transfer()), _tail(j._tail), _jp(j._jp), _par(j._par), _paridx(j._paridx)
    { // Note: yes, we want transfer(). finalize() would be wrong since it sets _j == _jp for this, which is fail
    }

    template<typename T>
    JobOp(const T& t) : Base(_Makejob(t)), _tail(this->_j), _jp(NULL), _par(NULL), _paridx(NULL)
    {
    }
    ...
    JobOp(Chain& c) : Base(c.transfer()), _tail(this->_j), _jp(NULL), _par(NULL), _paridx(NULL)
    {
    }

    JobOp(tws_Event *ev) : Base(_Eventjob(ev)), _tail(this->_j), _jp(NULL), _par(NULL), _paridx(NULL)
    {
    }

    JobOp(Event& ev) : Base(_Eventjob(ev)), _tail(this->_j), _jp(NULL), _par(NULL), _paridx(NULL)
    {
    }
...
public:
    ...
    {
        tws_Job *j = const_cast<JobOp*>(this)->transfer();
        TWS_ASSERT(j != _jp, "impossibru");
