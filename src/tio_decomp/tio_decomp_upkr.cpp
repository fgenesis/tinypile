...
out:
    ...
    if(err < 0)
    {
        // Calling fail() here would close the stream instantly and delete upk,
        // causing use-after-free when saving state upon yield.
        // The only good cause of action is to yield and fail on the next refill call.
    ...
    // src doesn't want to deliver. save state and get out.
        ...
        for(;;)
        {
            ...
            if(v) // is match?
            {
                ...
                if(offset > upk->windowSize)
                    return fail(sm, tio_Error_TooSmall);
                ...
                size_t copylen = v;
                const size_t mask = upk->windowSize - 1;
                while(copylen)
                {
                    const size_t back_ofs = (wpos - offset) & mask;
...
{
    tioUpkrStreamPriv* const upk = context(sm);
    tio__memset(&upk->probs[0], 128, sizeof(upk->probs));
    upk->bits = 0;
    upk->offset = 0;
    upk->prev_was_match = 0;
    upk->state = upkr_state_begin;
...
static tioUpkrStreamPriv *allocCtx(tio_Alloc alloc, void *allocUD, size_t windowsize)
{
    size_t allocsize = windowsize + sizeof(tioUpkrStreamPriv);
    return (tioUpkrStreamPriv*)alloc(allocUD, NULL, 0, allocsize);
}
...
static const tio_StreamImpl impl =
{
    close,
    NULL,
};
...
{
    windowSize = RoundUpToPowerOfTwo(windowSize);
    ...
    sm->cursor = NULL;
    sm->impl = &upkr::impl;
    sm->Refill = upkr::begin_decomp;
    sm->common.flags = flags;

    sm->priv.aux = packed;
    sm->priv.blockSize = windowSize;
    sm->priv.extra = priv;
