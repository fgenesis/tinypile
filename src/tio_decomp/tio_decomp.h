...
/* --- Stream-to-stream functions. See the note at the top for details. ---
    ...
    'packed' must stay alive while 'sm' is used, and not move in memory.
...
/* Decompress a upkr block. Upkr has an end marker so you don't need to pass a size,
   but you need to know the minimal window size that the data was compressed with.
   (Any larger window size will also work. If the passed window size is too small,
   the stream will eventually fail with tio_Error_TooSmall) */
